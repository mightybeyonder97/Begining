...
import activities.activity8.Searching;
...
public class Sorting {
	...
	void cutoffQsort(T[] data, int min, int max) {
		if (min < max) {
			...
			int indexOfPartition = partition(data, min, max);
			if (indexOfPartition-min < 7) {
				Sorting.selectionSort(data);
			}
			else  {
			quickSort(data, min, indexOfPartition - 1);
			} 
			if (max - indexOfPartition < 7) {
				Sorting.selectionSort(data);
			}
			else {
			quickSort(data, indexOfPartition + 1, max); 
	...
	List<Integer> closestPair(Integer[] data) {
		List<Integer> sorted  = new ArrayList<Integer>(2);
	...
	public static <T extends Comparable<T>> 
	List<Integer> firstN(Integer[] data, int n) {
		List<Integer> sElement =  new ArrayList<Integer>(n);
